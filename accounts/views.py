...
def home(request)
    try
        ...
        # A customer is overdue if they have a positive balance_amount.
        # Age of debt = days since their FIRST (oldest) milk entry,
        # because that is when the debt started accumulating.
        ...
        return render(request, 'accountshome.html', context)
    except Exception as e
        return render(request, 'accountshome.html', {
            'total_customers'   0,
            'total_litres'      0,
            'total_balance'     0,
            'total_amount'      0,
...
def customer_list(request)
    ...
    for customer in customers
        ...
        customer.total_ml = total_ml  # pyright ignore[reportAttributeAccessIssue]
        customer.total_litres = round(Decimal(total_ml)  Decimal(1000), 2) if total_ml else Decimal(0)  # pyright ignore[reportAttributeAccessIssue]
    return render(request, 'accountscustomer_list.html', {'customers' customers})
...
def customer_detail(request, customer_id)
    ...
    return render(request, 'accountscustomer_detail.html', context)
    ...
    if request.method == 'POST'
        ...
        if form.is_valid()
            ...
            if customer
                ...
                return redirect('accountscustomer_list')
    else
        form = MilkEntryForm()
    return render(request, 'accountsentry_form.html', {'form' form})
...
def edit_entry(request, entry_id)
    ...
    if request.method == 'POST'
        ...
        if form.is_valid()
            form.save()
            return redirect('accountscustomer_detail', customer_id=entry.customer.id)  # pyright ignore[reportAttributeAccessIssue]
    else
        form = MilkEntryForm(instance=entry)
    return render(request, 'accountsentry_form.html', {'form' form, 'title' 'Edit Milk Entry'})
...
def delete_entry(request, entry_id)
    entry = get_object_or_404(MilkEntry, id=entry_id)
    customer_id = entry.customer.id  # pyright ignore[reportAttributeAccessIssue]
    entry.delete()
    return redirect('accountscustomer_detail', customer_id=customer_id)
...
def edit_customer(request, customer_id)
    ...
    if request.method == 'POST'
        ...
        if form.is_valid()
            form.save()
            return redirect('accountscustomer_detail', customer_id=customer.id)  # pyright ignore[reportAttributeAccessIssue, reportOptionalMemberAccess]
    else
        form = CustomerForm(instance=customer)
    return render(request, 'accountscustomer_form.html', {'form' form, 'customer' customer, 'title' 'Edit Customer'})
...
def bill_pdf(request, customer_id, year=None, month=None)
    ...
    if year and month
        ...
        filename = fbill_{customer.name.replace(' ', '_')}_{year}_{month02d}  # pyright ignore[reportOptionalMemberAccess]
    else
        entries = MilkEntry.objects.filter(customer=customer).order_by('date')
        filename = fbill_{customer.name.replace(' ', '_')}_all  # pyright ignore[reportOptionalMemberAccess]
    ...
    response = HttpResponse(pdf_buffer.getvalue(), content_type='applicationpdf')
...
def monthly_summary(request)
    ...
    # Show years from the earliest entry up to current year
...
def upload_entries(request)
    if request.method == POST
        ...
        if not uploaded_file
            return render(request, accountsupload_entries.html, {error No file selected})

        path = default_storage.save(
            fuploads{uploaded_file.name},
        ...
        try
            ...
            return render(request, accountsupload_entries.html, {
                error fAI Processing Failed {str(e)}
            })

        request.session['ai_preview'] = extracted_data
        return redirect(accountspreview_entries)
...
def preview_entries(request)
    ...
    existing_entries = [[name, str(date)] for name, date in existing_entries]

    # BUG FIX json.dumps with default ensure_ascii=True emits uXXXX escape
    # sequences. When the template then applies escapejs, every backslash is
    # re-escaped, turning u0B95 into u0B95 so JSON.parse() throws a
    # SyntaxError for any Tamilnon-ASCII customer name.
    # Fix ensure_ascii=False keeps characters as-is; the template renders the
    # JSON directly into a script block using safe (we control this data).
    existing_entries_json = json.dumps(existing_entries, ensure_ascii=False)

    return render(request, accountspreview_entries.html, {
...
def confirm_entries(request)
    ...
    with transaction.atomic()
        for i in range(total_rows)
            try
                if not request.POST.get(finclude_{i})
                    continue

                customer_name = request.POST.get(fcustomer_{i}, ).strip()
                date_str = request.POST.get(fdate_{i})
                quantity_raw = request.POST.get(fquantity_{i}, 0)
                ...
                # BUG FIX get_or_create(name__iexact=...) puts a lookup
                # expression in the non-defaults kwargs which Django treats as a
                # field name, raising FieldError at runtime.
                # Correct pattern filter with the lookup, then create if absent.
                customer = (
    ...
    with open(audit_path, a, encoding=utf-8) as log
        log.write(json.dumps(audit_record) + n)
...
def _extract_from_image(file_path):
    ...
    # Merge: whichever pass captured more content leads; the other appended.
    # The LLM downstream resolves any duplicates or conflicts.
...
def _extract_from_pdf(file_path)
    ...
    Fall back to EasyOCR for scanned  image-only PDFs.

    BUG FIX page.extract_text() without layout=True collapses column-aligned
    rows (customer name on left, quantity on right) into a single stream,
    causing tokens to be reordered or dropped.  Two improvements
      1. extract_text(layout=True) — preserves x-position spacing so each
         name  spaces  quantity row stays intact on one line.
      2. words-based reconstruction — groups words by their top-y coordinate
         (rounded to 3 px) to rebuild rows even when layout=True still splits
         a line.  This guarantees that EVERY word that appears on the page is
         emitted in left-to-right, top-to-bottom order with no omissions.
    
    try
        import pdfplumber  # pyright ignore[reportMissingImports] # pip install pdfplumber
    except ImportError
    ...
    def _reconstruct_lines_from_words(page)
        ...
        Group words by their y-coordinate into logical rows, then sort each
        row left-to-right.  Returns the page as plain text with one row per
        line.  This is robust to irregular word spacing that confuses
        extract_text().
        
        words = page.extract_words(keep_blank_chars=False, x_tolerance=5, y_tolerance=5)
        if not words
            return 
        # Round y to nearest 3 px so words on the same visual row cluster together
        rows = {}
        for w in words
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf
        for page in pdf.pages
            # Attempt 1 layout-preserving extraction (keeps column spacing)
            text = page.extract_text(layout=True) or 
            # Attempt 2 words-based reconstruction (catches any remaining gaps)
            words_text = _reconstruct_lines_from_words(page)
            # Use whichever gave more content; prefer words_text as it never
            # drops tokens that are present on the page.
            best = words_text if len(words_text) = len(text) else text
            pages_text.append(best)
            print(f[PDF] Page extracted ({len(best)} chars)n{best[300]})

    text = n.join(pages_text).strip()
    ...
    for page_num, pil_img in enumerate(images, start=1):
        # Convert PIL image → PNG bytes (OCR.Space accepts PNG directly)
        buf = io.BytesIO()
        ...
        # Tamil pass (non-fatal if it fails)
        try:
...
def _extract_from_doc(file_path)
    Legacy .doc via antiword (Windows install separately).
    with default_storage.open(file_path, rb) as f
        doc_bytes = f.read()
    ...
    raise Exception(
        ...
        httpswww.winfield.demon.nl (Windows) or apt install antiword (Linux)
    )
    ...
    try
        ...
        for sheet in wb.worksheets
            for row in sheet.iter_rows(values_only=True)
                line = t.join(str(c) if c is not None else  for c in row)
                if line.strip()
                    rows.append(line)
        return n.join(rows)
    except Exception
...
def _extract_from_odt(file_path)
    try
        from odf import teletype  # pyright ignore[reportAttributeAccessIssue, reportMissingImports] # pip install odfpy
        from odf.opendocument import load as odf_load # pyright ignore[reportMissingImports]
    except ImportError
        raise Exception(odfpy is required for .odt extraction. Run pip install odfpy)

    with default_storage.open(file_path, rb) as f
        doc = odf_load(io.BytesIO(f.read()))
    return teletype.extractText(doc.text) # pyright ignore[reportAttributeAccessIssue]
...
def process_file_with_ai(file_path)
    ...
    if extractor is None
        # Last-ditch UTF-8 attempt for unknown text-like files
        try
            with default_storage.open(file_path, rb) as f
            ...
            raise Exception(
                fUnsupported file type .{extension}. 
...
{text_content}
    ...
    raw_output = response.choices[0].message.content.strip()  # pyright ignore[reportOptionalMemberAccess]
    ...
    # BUG FIX was using re.search(r[.], ..., re.DOTALL) — greedy regex fails
    # on nested structures and any trailing text after the closing ].
    # _extract_json_array uses a bracket-counting state machine that is correct.
    # Attempt 1 use the robust bracket-counter to find the JSON array
    parsed = _extract_json_array(clean_output)

    # Attempt 2 model returned a single object {...} instead of an array
    if parsed is None
    ...
    if parsed is None
        raise Exception(
            ...
            fRaw response (first 300 chars) {raw_output[300]!r}
        )
    preview_list = []
    ...
    for row in parsed
        ...
        if quantity  0 or quantity  50000
            ...
            confidence = 0.9  # 0 ml is valid per rules
...
# =========================================================
# 1. Add PaymentForm to the forms import line at the top
#    from .forms import MilkEntryForm, CustomerForm, PaymentForm
#
# 2. Add Payment to the models import line
#    from .models import Customer, MilkEntry, Payment, PRICE_PER_LITRE
#
# 3. Paste this view function anywhere in views.py
...
def add_payment(request, customer_id)
    ...
    # ── Step 1 Last month's date range ───────────────────────────────────
    today             = timezone.localdate()
    first_of_this_month = today.replace(day=1)
    last_month_end    = first_of_this_month - datetime.timedelta(days=1)
    last_month_start  = last_month_end.replace(day=1)

    # ── Step 2 Previous month milk total ─────────────────────────────────
    # MilkEntry has no total_amount field — calculate from quantity_ml × rate
    total_qty_ml = (
        MilkEntry.objects
        .filter(
            customer=customer,
    ...
    # ── Step 3 True total outstanding ────────────────────────────────────
    # carried balance  +  last month's new milk charges
    ...
    if request.method == 'POST'
        ...
        if form.is_valid()
            ...
            if paid_amount = 0
                ...
                # new balance = (old balance + prev month charges) − paid now
                ...
                return redirect('accountscustomer_detail', customer_id=customer.id) # type ignore
    else
        form = PaymentForm()
    ...
    return render(request, 'accountspayment_form.html', {
        'form'              form,
        'customer'          customer,
        'prev_month_total'  prev_month_total,   # ₹ last month's milk bill
        'total_outstanding' total_outstanding,   # ₹ what customer truly owes
...
def payment_history(request, customer_id=None)
    # ── Filter by customer if customer_id provided ────────────────────────
    ...
    # ── Optional filter by customer from GET param (dropdown) ────────────
    ...
    filter_month = request.GET.get('month')   # format YYYY-MM
    if filter_month
        try
    ...
    # ── Per-customer summary for the summary cards ────────────────────────
    customer_summary = (
        Payment.objects
    ...
    return render(request, 'accountspayment_history.html', context)
...
def edit_payment(request, payment_id)
    ...
    if request.method == 'POST'
        ...
        if form.is_valid()
            ...
            # Adjust customer balance undo old payment, apply new payment
            customer.balance_amount = customer.balance_amount + old_amount - new_amount
            customer.save()

            messages.success(
                request,
                f'✅ Payment updated. New balance ₹{customer.balance_amount.2f}'
            )
            return redirect('accountspayment_history')
    else
        form = PaymentForm(instance=payment)

    return render(request, 'accountsedit_payment.html', {
        'form'     form,
        'payment'  payment,
        'customer' customer,
    })
...
def delete_payment(request, payment_id)
    ...
    # Restore the balance (undo the payment)
    ...
    messages.success(
        request,
        f'🗑️ Payment of ₹{payment.amount.2f} deleted. '
        f'Balance restored to ₹{customer.balance_amount.2f}'
    )
    return redirect('accountspayment_history')


def calculator_view(request)
    return render(request, accountscalculator.html)
