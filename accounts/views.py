...
def edit_entry(request, entry_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('accounts:customer_detail', customer_id=entry.customer.id)  # pyright: ignore[reportAttributeAccessIssue]
    else:
...
def edit_customer(request, customer_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('accounts:customer_detail', customer_id=customer.id)  # pyright: ignore[reportAttributeAccessIssue, reportOptionalMemberAccess]
    else:
...
def bill_pdf(request, customer_id, year=None, month=None):
    ...
    if year and month:
        ...
        filename = f"bill_{customer.name.replace(' ', '_')}_{year}_{month:02d}"  # pyright: ignore[reportOptionalMemberAccess]
    else:
        entries = MilkEntry.objects.filter(customer=customer).order_by('date')
        filename = f"bill_{customer.name.replace(' ', '_')}_all"  # pyright: ignore[reportOptionalMemberAccess]
...
def preview_entries(request):
    ...
    # re-escaped, turning \u0B95 into \\u0B95 so JSON.parse() throws a
    # SyntaxError for any Tamil/non-ASCII customer name.
...
def _extract_from_image(file_path):
    """EasyOCR on preprocessed image — supports English + Tamil characters."""
    processed_img = preprocess_image_for_ocr(file_path)
    # BUG FIX: was ['en'] only — Tamil customer names were corrupted/dropped.
    # 'ta' adds Tamil script support; 'en' keeps English digits and labels.
    reader = easyocr.Reader(['en', 'ta'], gpu=False)
    results = reader.readtext(processed_img, detail=1)
    lines = []
    for (bbox, text, prob) in results:
        print(f"[OCR] Detected: {text!r}  conf={prob:.2f}")
        lines.append(text)
    text = "\n".join(lines)
    print("\n========== OCR TEXT ==========")
    print(text)
...
def _extract_from_pdf(file_path):
    ...
    print("[PDF] No selectable text — falling back to OCR per page")
    try:
        from pdf2image import convert_from_bytes  # pyright: ignore[reportMissingImports] # pip install pdf2image
    except ImportError:
        raise Exception(
            "pdf2image is required for scanned PDF OCR. "
            "Run: pip install pdf2image  (also needs: winget install poppler on Windows)"
        )

    images = convert_from_bytes(pdf_bytes, dpi=200)
    # BUG FIX: was ['en'] only — same multi-language fix as _extract_from_image
    reader = easyocr.Reader(['en', 'ta'], gpu=False)
    all_lines = []
    for img in images:
        img_np = np.array(img)
        results = reader.readtext(img_np, detail=1)
        all_lines.extend(t for (_, t, _) in results)
    return "\n".join(all_lines)
