...
def monthly_summary(request):
    ...
    def build_summary(start_date, end_date):
        ...
        return (
            summary_list,
            round(sum(item['amount'] for item in summary_list), 2),
            round(Decimal(total_ml_all) / Decimal(1000), 2),
        )
...
def edit_payment(request, payment_id):
    ...
    return render(request, 'accounts/edit_payment.html', {
        'form': form, 'payment': payment, 'customer': customer,
    })
...
def delete_payment(request, payment_id):
    ...
    messages.success(
        request,
        f'Payment of ₹{payment.amount:.2f} deleted. '
...
def _extract_from_pdf(file_path):
    ...
    try:
        ...
        raise Exception(
            ...
            "Run: pip install pdf2image  (server also needs: apt install poppler-utils)"
        )
...
def _extract_from_doc(file_path):
    """Legacy .doc via antiword (apt install antiword on Linux)."""
    with default_storage.open(file_path, "rb") as f:
        doc_bytes = f.read()
...
def _extract_json_array(text):
    ...
    Handles nested structures and escaped quotes inside strings.
